...
#define LEV_NUM 1024
uint32_t BUCK_NUM = (3000);
#define HASH_NUM 3
#define BURST_THRE (100*unittime)
#define DECAY 1
#define stair 8
#define DECAY_CYCLE (unittime/0.5)
#define LEV 1.1
#define BURST_LEV 5
    ...
    void insert(const uint64_t flow, const uint64_t & currTime){
        ...
        rep2(i, 0, HASH_NUM){
            ...
            if(flow == 113254552UL){
                ...
                if(bucket[pos]->last_sm+bucket[pos]->speed_monitor < min_value) min_value = bucket[pos]->last_sm+bucket[pos]->speed_monitor;
            }
        }
        if(flow == 113254552UL){
            gtlog << currTime << ','<< min_value << endl;
        }
    }
